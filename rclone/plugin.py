...
menu = {
    'main' : [package_name, 'RClone'],
    'sub' : [
        ['setting', '설정'], ['status', '상태'], ['list', '목록'], ['mount', 'Mount'], ['serve_setting', 'Serve'], ['log', '로그']
    ]
}
...
def detail(sub): 
    ...
    if sub == 'setting':
        ...
        arg = {}
        arg['option'] = '--allow-other --fast-list --drive-skip-gdocs --poll-interval=1m --buffer-size=32M --vfs-read-chunk-size=32M --vfs-read-chunk-size-limit 2048M --vfs-cache-mode writes --dir-cache-time=1h --log-level INFO'
        #if platform.system() != 'Windows':
        #    arg['option'] += ' --daemon'
        return render_template('%s_%s.html' % (package_name, sub), arg=arg)
    elif sub == 'serve_setting':
        arg = {}
        arg['option'] = '--user sjva --pass sjva --fast-list --drive-skip-gdocs --poll-interval=1m --buffer-size=32M --vfs-read-chunk-size=32M --vfs-read-chunk-size-limit 2048M --vfs-cache-mode writes --dir-cache-time=1h --log-level INFO'
        return render_template('%s_%s.html' % (package_name, sub), arg=arg)
    else:
